...
class HomeFragment : Fragment() {
    ...
    private fun setupRecyclerViews() {
        ...
        binding.rvHorizontalHome.layoutManager =
    ...
    private fun setupObservers() {
        homeViewModel.upcomingEvents.observe(viewLifecycleOwner) { upcomingEvents ->
            val upcomingEvents = upcomingEvents.take(5)
            colAdapter.submitList(upcomingEvents)
        }

        homeViewModel.finishedEvents.observe(viewLifecycleOwner) { finishedEvents ->
            val finishedEvents = finishedEvents.take(5)
            rowAdapter.submitList(finishedEvents)
        }
